...
You are searching for a node in a JSON graph and you have one Input which you must use to locate the node in the JSON structure.

The Input can have spelling mistakes, be multiple words or a phrase, or be a single word or numbers.

In the JSON graph structure, you will find the format:
{"concepts": [{"concept_id": int,"concept_name": str,"importance": float,"relationships": [{"target_id": int,"relationship": str,"weight": float}]}]}

*** Your Task: You must semantically understand the Input and provide the most likely concept_id associated with that Input. ***

*** Do NOT return a concept_id that does not exist in the JSON ***
*** Return ONLY a single integer as a string (e.g. "2") ***
*** Do NOT include explanations, reasoning, or commentary ***
*** Do NOT return JSON ***
*** Do NOT return multiple integers ***
...
def generate(req: GenerateGraph):
	return {"output": generate_graph(req.rawNotes)}
...
def summarise(req: GenerateSummary):
    return generate_summary(req.rawNotes, req.graphJson, req.userSummary)

### ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

## AI Searching JSON Graph ~~~~~~~~~

def search_graph(search_input: str, json_graph: str) -> int:
    developed_prompt = (
        graph_searching_template + "\n<<<"
        + "\nInput:\n" + search_input
        + "\nJSON:\n" + json_graph + "\n>>>"
    ...
    parsed = int(json.loads(data.get("response", "").strip()))

    return parsed

@router.post("/semantic-search")
def search(req: SearchGraph):
    return {"output": search_graph(req.search_input, req.graphJson)}

"""
    // Hans AI SEARCH //
    const aiResponse = await fetch("http://localhost:8000/ai/semantic-search", {
        method: "POST",
    ...
    const data = await aiResponse.json(); // produces: { "output": 2 }
"""
